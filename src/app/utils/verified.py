...
def verify_answer(user_message: str, draft_text: str, min_citations: int = 1) -> Dict[str, Any]:
    ...
    if not GEMINI_API_KEY:
        return {"answer": draft_text or "", "sources": [], "verified": False, "reasoning_note": "No API key configured"}
    try:
        model = _model()
        prompt = (
            "User message:\n" + (user_message or "") +
        ...
        data = _safe_json(raw) or {"answer": draft_text or "", "sources": [], "verified": False, "reasoning_note": "Could not parse JSON"}
        if isinstance(data, dict):
            ...
            data = {"answer": draft_text or "", "sources": [], "verified": False, "reasoning_note": "Invalid JSON"}
        return data
    except Exception as e:
        return {"answer": draft_text or "", "sources": [], "verified": False, "reasoning_note": f"Verification error: {e}"}
...
def verify_with_gemini_v2(prompt: str, citations: bool = True):
    ...
    try: _ver_metrics.token_budget_remaining.dec(float(prompt_tokens))
    ...
    try: _ver_metrics.token_budget_remaining.dec(float(completion_tokens))
    except Exception: pass
