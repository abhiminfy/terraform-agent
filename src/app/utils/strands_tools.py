...
class TerraformValidatorTool:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    result = subprocess.run(
                        ["terraform", "validate", "-json"],  # Use JSON output for better parsing
                        cwd=temp_dir,
                        capture_output=True,
                        text=True,
                        timeout=timeout
                        ...
                        if result.stdout:
                            ...
                            if "diagnostics" in json_output:
                                ...
                                for diag in json_output["diagnostics"]:
                                    ...
                                    error_details.append(f"{summary}: {detail}" if detail else summary)
                                error_msg = "\n".join(error_details)
                    except json.JSONDecodeError:
                        pass
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    logger.warning(f"Terraform fmt timed out after {timeout} seconds, returning original")
                    return terraform_code
    ...
    def autofix_common_hcl(code: str, region: str | None = None):
        ...
        c2 = re.sub(r'data\s+"aws_default_vpc"\s+"default"\s*{[^}]*}',
        ...
        if m:
            sg = m.group(1)
            code = re.sub(r'aws_security_group\.\w+\.id',
                          f'aws_security_group.{sg}.id', code)

        # apply explicit provider region if provided
        if region:
            code = re.sub(r'provider\s+"aws"\s*{[^}]*}',
    ...
    def run_fmt_validate_plan_from_code(terraform_code: str, timeout: int = 120) -> Dict[str, Any]:
        ...
        def run(cmd, cwd):
            try:
                ...
                return {"rc": 124, "out": "", "err": f"timeout after {timeout}s: {' '.join(cmd)}"}
        ...
        with tempfile.TemporaryDirectory() as work:
            ...
            plan = run(["terraform", "plan", "-detailed-exitcode", "-no-color", "-input=false"], work)
            ...
            m = re.search(r"Plan:\s+\d+\s+to\s+add,\s+\d+\s+to\s+change,\s+\d+\s+to\s+destroy", out["plan_output"])
            if m:
...
class AWSPricingTool:
    ...
    @staticmethod
    def estimate_ec2_cost(instance_type: str, count: int = 1, hours_per_month: int = 730) -> Dict[str, Any]:
        """Estimate EC2 instance costs"""
        if instance_type not in AWSPricingTool.PRICING_DATA["ec2"]:
            return {"error": f"Unknown instance type: {instance_type}. Available types: {list(AWSPricingTool.PRICING_DATA['ec2'].keys())}"}
    ...
    @staticmethod
    def estimate_rds_cost(db_instance_class: str, storage_gb: int = 20, storage_type: str = "gp2") -> Dict[str, Any]:
        """Estimate RDS costs"""
        if db_instance_class not in AWSPricingTool.PRICING_DATA["rds"]:
            return {"error": f"Unknown DB instance class: {db_instance_class}. Available types: {list(AWSPricingTool.PRICING_DATA['rds'].keys())}"}
    ...
    @staticmethod
    def estimate_s3_cost(storage_gb: int, storage_class: str = "standard", requests_per_month: int = 0) -> Dict[str, Any]:
        """Estimate S3 storage costs"""
        if storage_class not in AWSPricingTool.PRICING_DATA["s3"]:
            return {"error": f"Unknown storage class: {storage_class}. Available types: {list(AWSPricingTool.PRICING_DATA['s3'].keys())}"}
    ...
    @staticmethod
    def estimate_vpc_cost(nat_gateways: int = 0, elastic_ips: int = 0, data_transfer_gb: int = 0) -> Dict[str, Any]:
...
*.temp"""
        ...
        try:
            ...
            return {"success": True, "message": f".gitignore created successfully for {project_type} project"}
        except Exception as e:
        ...
        try:
            ...
            try:
                subprocess.run(["git", "remote", "add", "origin", repo_url], 
                             check=True, capture_output=True, timeout=timeout//4)
            except subprocess.CalledProcessError:
                # Remote might already exist, try to set URL
                subprocess.run(["git", "remote", "set-url", "origin", repo_url], 
                             check=True, timeout=timeout//4)
            except subprocess.TimeoutExpired:
            ...
            try:
                ...
                return {"success": True, "message": f"Successfully pushed to GitHub: {repo_url}", "output": push_result.stdout}
            except subprocess.TimeoutExpired:
                return {"success": False, "error": f"Git push timed out after {timeout} seconds"}
...
class InfrastructureAnalyzer:
    ...
    def analyze_terraform_resources(terraform_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            security_checks = [
                ...
                (r'password\s*=\s*"[^"]*"', "🔐 Hardcoded password detected - use variables or AWS Secrets Manager", "HIGH"),
                (r'secret\s*=\s*"[^"]*"', "🔐 Hardcoded secret detected - use AWS Secrets Manager", "HIGH"),
                (r'access_key\s*=\s*"[^"]*"', "🔐 Hardcoded access key - use IAM roles instead", "CRITICAL"),
                (r'secret_key\s*=\s*"[^"]*"', "🔐 Hardcoded secret key - use IAM roles instead", "CRITICAL"),
                
                # Network security
                (r'cidr_blocks\s*=\s*\["0\.0\.0\.0/0"\]', "🌐 Open security group rule (0.0.0.0/0) - restrict access", "HIGH"),
                (r'from_port\s*=\s*0.*to_port\s*=\s*65535', "🌐 Security group allows all ports - restrict port range", "MEDIUM"),
                (r'protocol\s*=\s*"-1"', "🌐 Security group allows all protocols - be more specific", "MEDIUM"),
                
                # Database security
                (r'publicly_accessible\s*=\s*true', "🗄️ Database publicly accessible - consider private subnets", "HIGH"),
                (r'skip_final_snapshot\s*=\s*true', "🗄️ Database skip final snapshot - data loss risk", "MEDIUM"),
                (r'deletion_protection\s*=\s*false', "🗄️ Deletion protection disabled - enable for production", "HIGH"),
                
                # Storage security
                (r'versioning\s*\{\s*enabled\s*=\s*false', "🪣 S3 versioning disabled - enable for data protection", "MEDIUM"),
                (r'force_destroy\s*=\s*true', "🪣 S3 force destroy enabled - data loss risk", "HIGH"),
                (r'block_public_acls\s*=\s*false', "🪣 S3 public ACLs allowed - security risk", "HIGH"),
                
                # Encryption
                (r'encrypted\s*=\s*false', "🔒 Encryption disabled - enable encryption at rest", "HIGH"),
                (r'server_side_encryption_configuration\s*\{\s*\}', "🔒 S3 encryption not configured", "MEDIUM"),
                
                # Instance security
                (r'associate_public_ip_address\s*=\s*true', "🖥️ Instance gets public IP - consider private subnets", "MEDIUM"),
                (r'source_dest_check\s*=\s*false', "🖥️ Source/dest check disabled - security risk", "MEDIUM"),
                
                # IAM security
                (r'Effect"\s*:\s*"Allow".*"Resource"\s*:\s*"\*"', "👤 IAM policy allows all resources - use least privilege", "HIGH"),
                (r'"Action"\s*:\s*"\*"', "👤 IAM policy allows all actions - use least privilege", "CRITICAL"),
            ]
            ...
            blast_radius = 0
            critical_count = len([c for c in analysis["security_concerns"] if c.get("severity") == "CRITICAL"])
            high_count = len([c for c in analysis["security_concerns"] if c.get("severity") == "HIGH"])
            medium_count = len([c for c in analysis["security_concerns"] if c.get("severity") == "MEDIUM"])
            ...
            if not any("encryption" in concern["message"].lower() for concern in analysis["security_concerns"]):
            ...
            if any(res["type"] in ["aws_rds_instance", "aws_db_instance"] for res in analysis["resources"]):
                if any("publicly_accessible" in concern["message"] for concern in analysis["security_concerns"]):
            ...
            analysis["optimization_suggestions"] = InfrastructureAnalyzer._generate_optimization_suggestions(
    ...
    def _generate_optimization_suggestions(terraform_code: str, resources: List[dict]) -> List[str]:
        ...
        # Tagging suggestions
        if "tags" not in terraform_code and any(res["type"].startswith("aws_") for res in resources):
            suggestions.append("🏷️ Add consistent resource tags for cost tracking and management")
        
        # State management suggestions
        if "backend" not in terraform_code:
            suggestions.append("💾 Configure remote state backend (S3 + DynamoDB) for team collaboration")
        ...
        if "aws_instance" in [r["type"] for r in resources] and "availability_zone" not in terraform_code:
    ...
    def suggest_best_practices(terraform_code: str) -> List[str]:
        ...
        if "tags" not in terraform_code and "aws_" in terraform_code:
            suggestions.append("Add consistent tags to AWS resources for better management and cost tracking")
        ...
        if "data." not in terraform_code and len(re.findall(r'resource\s+"aws_', terraform_code)) > 3:
...
class FileOperations:
    ...
    @staticmethod
    def create_terraform_structure(project_name: str = "terraform-project") -> Dict[str, Any]:
...
"""
            ...
            for env in ["dev", "staging", "prod"]:
                ...
                (env_dir / "terraform.tfvars").write_text(f"""# {env.title()} environment variables
aws_region   = "us-east-1"
environment  = "{env}"
project_name = "{project_name}-{env}"
""", encoding='utf-8')
                (env_dir / "main.tf").write_text(f"""# {env.title()} environment configuration
terraform {{
  backend "s3" {{
...
def static_sanity_checks(hcl_text: str) -> List[str]:
    ...
    if not re.search(r'\\bresource\\s+"aws_lb_target_group"\\b', txt):
        issues.append('Missing target group: add resource "aws_lb_target_group" with target_type="ip" for Fargate.')
    ...
    if re.search(r'\\bresource\\s+"aws_ecs_service"\\b', txt):
        ...
        m = re.search(r'resource\\s+"aws_ecs_service"\\s+"[^"]+"\\s*{[^}]*network_configuration\\s*{(?P<body>[^}]*)}', txt, re.S)
        if m:
            net = m.group("body")
            ...
            if re.search(r'aws_subnet\\.public', net):
                issues.append("ECS service should use private subnets (not public) in network_configuration.subnets.")
        else:
    ...
    if re.search(r'com\\.amazonaws\\.(?:\\$\\{[^}]+}|[a-z0-9-]+)\\.s3', txt):
        ...
        if re.search(r'route_table_ids\\s*=\\s*\\[\\s*aws_route_table\\.public(?:\\[.*?\\])?\\.id\\s*\\]', txt):
            issues.append("Attach the S3 Gateway endpoint to PRIVATE route tables, not the public route table.")

    # 6) ECS SG heuristic (allow from ALB SG)
    if re.search(r'resource\\s+"aws_security_group"\\s+"ecs', txt) and "source_security_group_id" not in txt:
        issues.append("ECS SG should allow inbound from ALB SG via source_security_group_id (heuristic).")
...
def extract_best_terraform_block(response_text: str) -> str:
    ...
    labelled = re.findall(r"```(?:terraform|hcl|tf)\s*\n(.*?)```", response_text, re.DOTALL | re.IGNORECASE)
    ...
    hcl_spans = re.findall(r'((?:terraform|provider|variable|output|data|module|resource)\s+[^{]*\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\})', response_text, re.DOTALL | re.IGNORECASE)
    candidates.extend(hcl_spans)
    if not candidates:
        return ""
    # score by size + terraform-y tokens
    def score(txt: str) -> int:
        tokens = sum(1 for t in ["resource ", "provider ", "variable ", "output ", "module ", "terraform "] if t in txt)
...
from src.app.utils.utils import run_cmd_async as _st_run_cmd_async, secure_tempdir as _st_secure_tempdir
...
class TerraformValidateTool(Tool):
    ...
    async def run(self, tf_code: str):
        ...
        with _st_secure_tempdir("tf_val_v2_") as d:
            import os as _os
            with open(_os.path.join(d, "main.tf"), "w", encoding="utf-8") as f: f.write(tf_code)
            rc, out, err = await _st_run_cmd_async(_ST.TF_BIN, "init", "-input=false", cwd=d)
            if rc != 0: return {"success": False, "data": {"stderr": err}, "error": "terraform init failed"}
            rc, out, err = await _st_run_cmd_async(_ST.TF_BIN, "validate", cwd=d)
            ok = rc == 0
            return {"success": ok, "data": {"stdout": out, "stderr": err}, "error": "" if ok else err}

class AzureTool(Tool):
    name = "azure_inspect"
    async def run(self, subscription_id: str = ""):
        rc, out, err = await _st_run_cmd_async("az", "account", "show")
        return {"success": rc == 0, "data": {"subscription": out}, "error": "" if rc == 0 else err}

class GcpTool(Tool):
    name = "gcp_inspect"
    async def run(self, project_id: str = ""):
        rc, out, err = await _st_run_cmd_async("gcloud", "config", "list")
        return {"success": rc == 0, "data": {"config": out}, "error": "" if rc == 0 else err}
