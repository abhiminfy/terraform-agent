from strands_tools import (
    calculator, terraform_validator, infrastructure_analyzer,
    file_operations, plan_analyzer, static_sanity_checks
)
...
def infer_cloud(s: str) -> Optional[str]:
    s = (s or "").lower()
    if any(k in s for k in ["s3", "ec2", "iam", "vpc", "route53", "aws", "cloudwatch", "alb", "eks", "lambda"]):
        return "aws"
    if any(k in s for k in ["gcs", "gke", "bigquery", "pubsub", "gcp", "google cloud", "compute engine", "cloud run"]):
        return "gcp"
    if any(k in s for k in ["azure", "azurerm", "resource group", "storage account", "vnet", "aks"]):
        return "azure"
    return None
...
'''
    ...
    def run_with_timeout(command: List[str], timeout: int = 30, cwd: str = ".") -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "stdout": "", "stderr": f"Command timed out after {timeout} seconds", "returncode": -1, "timeout": True}
        except FileNotFoundError:
            tool_name = command[0] if command else "unknown"
            return {"success": False, "stdout": "", "stderr": f"Tool '{tool_name}' not found - please install it", "returncode": -2, "tool_missing": True}
        except Exception as e:
            return {"success": False, "stdout": "", "stderr": f"Command failed: {str(e)}", "returncode": -3}
                ...
                if result.get("success"):
                    ...
                    if budget:
                        ...
                        if alert != "GREEN":
                            ...
                            lines.append(f"Budget Utilization: {budget.get('budget_utilization_percent', 0):.1f}%")
                            lines.append("")
    ...
    def get_rough_cost_estimate(terraform_code: str) -> str:
        ...
        try:
            analysis = infrastructure_analyzer.analyze_terraform_resources(terraform_code)
            ...
            cost_breakdown.append("📊 ROUGH COST ESTIMATE (Not exact - for planning purposes only)")
            ...
            cost_breakdown.append(f"💰 Estimated yearly cost: ~${estimated_cost * 12:.2f}")
            cost_breakdown.append("")
            cost_breakdown.append("⚠️  This is a rough estimate. Install Infracost CLI for accurate pricing.")
            return "\n".join(cost_breakdown)
        except Exception as e:
    ...
    def analyze_blast_radius(terraform_code: str) -> List[str]:
        ...
        warnings = []
        dangerous_patterns = [
            (r'skip_final_snapshot\s*=\s*true', "⚠️ Database will be deleted without final snapshot"),
            ...
            (r'cidr_blocks\s*=\s*\["0\.0\.0\.0/0"\]', "⚠️ Security group allows access from anywhere (0.0.0.0/0)"),
    ...
    def is_infra_prompt(user_prompt: str) -> bool:
        ...
        infra_keywords = [
            "vpc", "ec2", "s3", "rds", "terraform", "provision", "infra",
            "deploy", "aws", "cloud", "instance", "database", "server",
            "load balancer", "security group", "subnet", "internet gateway",
            "elastic", "lambda", "cloudformation", "kubernetes", "docker",
            "create", "build", "setup", "configure", "gcp", "azure", "azurerm", "google"
        ]
    ...
    def mentions_cost_estimate(user_prompt: str) -> bool:
        if not user_prompt:
            return False
        lower = user_prompt.lower()
        cost_keywords = ["cost", "estimate", "infracost", "price", "pricing", "budget", "expensive", "cheap", "money"]
    ...
    def mentions_github_push(user_prompt: str) -> bool:
        if not user_prompt:
            return False
        lower = user_prompt.lower()
        git_keywords = ["github", "push", "commit", "repository", "repo", "git", "pull request", "pr"]
...
class TerraformAgent:
    ...
    def __init__(self):
        try:
            generation_config = {"temperature": 0.7, "top_p": 0.8, "top_k": 40, "max_output_tokens": 3072, "candidate_count": 1}
            safety_settings = [
    ...
    async def get_reasoned_response(
        self, user_prompt: str, chat_id: str = None, stream_thinking: bool = False, cloud_hint: Optional[str] = None
    ) -> Tuple[str, str]:
...
FINAL RESPONSE: <your response or Terraform code>"""
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if response and hasattr(response, 'text') and response.text:
                        ...
                        filtered_text, warnings = self.guardrails.filter_hallucinations(full_text, user_prompt)
                        ...
                        if attempt < max_retries - 1:
                            ...
                            return "Could not get response from AI.", "I'm having trouble generating a response right now. Please try again."
            ...
            return f"Error generating response: {str(e)}", "I encountered an error while processing your request. Please try again."
...
Return numbered questions only."""
        try:
            response = await asyncio.to_thread(self.model.generate_content, clarification_prompt)
            if response and hasattr(response, 'text') and response.text:
    ...
    async def process_message(self, user_message: str, chat_id: str = "default") -> Dict[str, Any]:
        ...
        try:
            ...
            should_clarify, clarification_questions = self.guardrails.should_ask_for_clarification(user_message)
            if should_clarify and self.guardrails.is_infrastructure_request(user_message):
                ...
                self.chat_memory.save_message(chat_id, "assistant", f"I need more information: {questions_text}")
                result.update({
                    "type": "clarify",
            ...
            if self.guardrails.is_infrastructure_request(user_message):
                ...
                if not raw_code or not re.search(r'\b(resource|provider|module)\b', raw_code, re.IGNORECASE):
                    print("❓ Need clarification for infrastructure request")
                    questions = await self.ask_for_clarity(user_message)
                    self.chat_memory.save_message(chat_id, "assistant", f"I need clarification: {questions}")
                    result.update({
                        "type": "clarify",
                        "content": "I need more information to help you better.",
                        "questions": questions,
                    ...
                    terraform_validation = self.guardrails.validate_terraform_response(model_response)
                    if not terraform_validation["valid"] or terraform_validation["confidence"] < 0.3:
                        result.update({
                            "type": "error",
                    ...
                    if policy_result.get("violations"):
                        comprehensive_report = self.policy_engine.create_policy_report(policy_result, processed_code)
                    ...
                    formatted_code = terraform_validator.format_terraform_code(processed_code)
                    validation_result = terraform_validator.validate_terraform_syntax(formatted_code)

                    want_plan = any(k in user_message.lower() for k in (
                        "plan", "show the plan", "review changes", "diff", "terraform plan"
                    ))
                    cli_checks: Dict[str, Any] = {}
                    if want_plan:
                        try:
                            cli_checks = terraform_validator.run_fmt_validate_plan_from_code(
                                formatted_code, timeout=120
                            )
                        except Exception as e:
                            cli_checks = {"errors": [f"plan-exec failed: {e}"], "plan_output": "", "plan_exit_code": 1}
                    ...
                    if validation_result["valid"]:
                        practices = infrastructure_analyzer.suggest_best_practices(formatted_code)
                    ...
                    if self.tools.mentions_cost_estimate(user_message):
                        ...
                        if self.infracost_integration.infracost_available:
                            cost_result = self.infracost_integration.generate_cost_estimate(formatted_code, "default")
                            if cost_result.get("success"):
                                ...
                                cost_estimate = self.tools.estimate_infracost_with_fallback(formatted_code)
                        else:
                            cost_estimate = self.tools.estimate_infracost_with_fallback(formatted_code)
                    ...
                    self.chat_memory.save_message(chat_id, "assistant", response_content, {
                        "type": "terraform",
                        "resource_count": len(security_analysis.get('resources', [])),
                    ...
                    result.update({
                        ...
                        "terraform_code": processed_code,
                        "formatted_code": formatted_code,
                        "validation_output": (cli_checks.get("plan_output") if cli_checks else validation_result.get("output", "Valid")),
                        "security_findings": security_concerns + [{"message": issue["description"], "severity": issue["severity"]} for issue in security_issues],
                ...
                self.chat_memory.save_message(chat_id, "assistant", response, {"type": "chat", "confidence": confidence})
                result.update({"type": "chat", "content": response, "thinking_trace": thinking, "confidence_score": confidence})
    ...
    def _auto_pin_required_version(self, files: dict) -> dict:
        ...
        def inject_or_update(content: str) -> str:
            if re.search(r'required_version\s*=\s*".*?"', content):
                return re.sub(r'required_version\s*=\s*".*?"', f'required_version = "{pin}"', content)
            if re.search(r'\bterraform\s*{', content):
                return re.sub(r'(\bterraform\s*{)', r'\1\n  required_version = "' + pin + '"', content, count=1)
            return (
                'terraform {\n'
...
def _sa_context(chat_id: str, n: int = 8) -> str:
    msgs = _sa_cache.get(chat_id) or []
    msgs = msgs[-n:]
    return "\n".join([f"{r.upper()}: {t}" for r, t in msgs]) if msgs else "(no prior context)"

def _sa_fallback(prompt: str) -> str:
    if not _sa_pipeline:
        return "Fallback model unavailable."
    gen = _sa_pipeline("text-generation", model=_sa_settings.HF_MODEL, max_new_tokens=256)
...
async def chat_secure(chat_id: str, user_text: str):
    ...
    pt = len(prompt.split()); _sa_metrics.model_tokens.labels(model_name="gemini-pro", type="prompt").inc(pt)
    try: _sa_metrics.token_budget_remaining.dec(float(pt))
    ...
    ct = len(answer.split()); _sa_metrics.model_tokens.labels(model_name="gemini-pro", type="completion").inc(ct)
    try: _sa_metrics.token_budget_remaining.dec(float(ct))
    except Exception: pass
