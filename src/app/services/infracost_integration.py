...
class InfracostIntegration:
    ...
    def generate_cost_estimate(self, terraform_code: str, workspace: str = "default") -> Dict[str, Any]:
...
""", encoding="utf-8")
    ...
    def _process_infracost_output(self, infracost_data: Dict[str, Any], workspace: str) -> Dict[str, Any]:
    ...
    def _check_budget_compliance(self, estimated_cost: float, workspace: str) -> Dict[str, Any]:
        ...
        if not workspace_budget:
            workspace_budget = self.budgets.get("workspaces", {}).get("default", {
                "monthly_limit": 100.0,
                "alert_thresholds": [50.0, 80.0, 100.0],
                "currency": "USD"
            })
    ...
    def _generate_budget_recommendations(self, cost: float, limit: float, over_budget: bool) -> List[str]:
        """Generate budget-related recommendations"""
        recommendations = []
        
        if over_budget:
            excess = cost - limit
            recommendations.append(f"[COST] BUDGET EXCEEDED: Estimated cost is ${excess:.2f} over budget")
        ...
        if utilization > 80:
            ...
            recommendations.append("[METRICS] Review most expensive resources for right-sizing opportunities")
        
        if cost > 500:
            recommendations.append(" Consider Reserved Instances or Savings Plans for long-term workloads")
    ...
    def generate_cost_diff(self, old_terraform: str, new_terraform: str, workspace: str = "default") -> Dict[str, Any]:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                diff_result = subprocess.run(
                    ["infracost", "diff", "--path", str(old_dir), "--compare-to", str(new_dir), "--format", "json"],
                    capture_output=True,
                    text=True,
                    timeout=120
                )
    ...
    def _process_cost_diff(self, diff_data: Dict[str, Any], workspace: str) -> Dict[str, Any]:
        ...
        if not projects:
            return {
                "success": False,
                "error": "No cost diff data available"
            }
            ...
            for resource in resources:
                ...
                if abs(monthly_diff) > 0.01:  # Only include significant changes
                    resource_diffs.append({
                        ...
                        "change_type": "added" if monthly_diff > 0 else "removed" if monthly_diff < 0 else "modified"
                    })
    ...
    def _calculate_percentage_change(self, diff_data: Dict[str, Any]) -> Optional[float]:
        ...
        try:
            ...
            if projects:
                ...
                if total_previous > 0:
                    return round(((total_current - total_previous) / total_previous) * 100, 1)
        except (KeyError, ValueError, ZeroDivisionError):
            pass
        return None
    
    def _generate_diff_summary(self, total_diff: float, resource_diffs: List[Dict[str, Any]]) -> str:
        ...
        if resource_diffs:
            ...
            if removed_count > 0:
                summary += f", {removed_count} resources removed" if added_count > 0 else f" ({removed_count} resources removed"
            if added_count > 0 or removed_count > 0:
                summary += ")"
        
        return summary
    
    def update_budget(self, workspace: str, monthly_limit: float, alert_thresholds: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Failed to update budget: {e}"
            }
...
from backend.app.utils.utils import run_cmd_async as _ic_run_cmd_async, secure_tempdir as _ic_secure_tempdir
...
_IC_RATES = {("USD","USD"):1.0, ("USD","INR"):83.0, ("USD","EUR"):0.9, ("EUR","USD"):1.11, ("INR","USD"):1/83.0}
    ...
    try:
        with _ic_secure_tempdir("ic_v2_") as d:
            ...
            with open(_os.path.join(d, "main.tf"), "w", encoding="utf-8") as f:
                f.write(tf_code)
            rc, out, err = await _ic_run_cmd_async(_ic_settings.TF_BIN, "init", "-input=false", cwd=d)
            if rc != 0:
                return {"success": False, "data": {"stderr": err}, "error": "terraform init failed"}
            rc, out, err = await _ic_run_cmd_async(_ic_settings.TF_BIN, "plan", "-out=plan.out", cwd=d)
            if rc != 0:
                return {"success": False, "data": {"stderr": err}, "error": "terraform plan failed"}
            rc, out, err = await _ic_run_cmd_async(_ic_settings.INFRACOST_BIN, "breakdown", f"--path={d}", "--format=json", cwd=d)
            if rc != 0:
                return {"success": False, "data": {"stderr": err}, "error": "infracost breakdown failed"}
            report = _json.loads(out or "{}")
            total_usd = float(report.get("totalMonthlyCost", 0.0) or 0.0)
            total_conv = _ic_convert(total_usd, currency)
            data = {"currency": currency.upper(), "total_monthly_cost_converted": total_conv, "total_monthly_cost_usd": total_usd, "breakdown": report}
            _ic_cache[key] = data
