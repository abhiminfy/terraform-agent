...
class PolicyEngine:
                ...
                try:
                    ...
                    for issue in results:
                        violations.append({
                            ...
                            "fix_suggestion": issue.get("resolution", "Review and fix security issue")
                        })
                ...
                try:
                    ...
                    for check in failed_checks:
                        violations.append({
                            ...
                            "severity": self._normalize_severity("HIGH"),  # Checkov doesn't provide severity
                            "message": check.get("check_name", "Checkov security check failed"),
                            "scanner": "checkov",
                            "category": "security",
                            "file": check.get("file_path"),
                            "line": check.get("file_line_range", [0])[0],
                            "fix_suggestion": check.get("guideline", "Review Checkov documentation")
                        })
                ...
                try:
                    ...
                    for issue in issues:
                        violations.append({
                            ...
                            "severity": self._normalize_severity(issue.get("rule", {}).get("severity", "MEDIUM")),
    ...
    def create_policy_report(self, validation_result: Dict[str, Any], terraform_code: str) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self, violations: List[Dict[str, Any]]) -> List[str]:
        ...
        if critical_violations:
            recommendations.append("[CRITICAL] URGENT: Address critical security violations before deployment")
        
        high_violations = [v for v in violations if v.get("severity") == "HIGH"]
        if high_violations:
            recommendations.append("[WARN] HIGH PRIORITY: Review and fix high-severity security issues")
        ...
        if "aws_security_groups" in str(violation_types):
            recommendations.append("[SECURE] Review security group rules and apply principle of least privilege")
        
        if "aws_s3_buckets" in str(violation_types):
            recommendations.append("[S3] Enable S3 security features: versioning, encryption, and access controls")
        
        if any("hardcoded" in v.get("message", "").lower() for v in violations):
            recommendations.append("[SECRET] Remove hardcoded credentials and use proper secret management")
...
from backend.app.utils.utils import run_cmd_async as _pe_run_cmd_async, secure_tempdir as _pe_secure_tempdir

_SOC2_CONTROLS = {
    "network_segmentation": "Security groups should restrict ingress/egress broadly."
}
_HIPAA_CONTROLS = {
    "https_only": "ALB/CloudFront listeners should enforce HTTPS."
}
...
async def validate_terraform_code_ast(tf_code: str) -> _Dict[str, _Any]:
    ...
    if "__parse_error__" in ast:
        return {"success": False, "data": {}, "error": f"HCL parse failed: {ast['__parse_error__']}"}
    ...
    return {"success": True, "data": {"findings": findings, "controls": controls, "tfsec": tfsec, "checkov": checkov}, "error": ""}
