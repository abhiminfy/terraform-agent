...
class GitHubIntegration:
    ...
    def __init__(self):
        ...
        if self.available:
            ...
            logger.warning("GitHub integration not available - missing token or repo configuration")
    ...
    def get_pr_status(self, pr_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to get PR status: {r.status_code} - {r.text}"}
        except Exception as e:
    ...
    def merge_pr(self, pr_number: int, merge_method: str = "squash") -> Dict[str, Any]:
        ...
        try:
            ...
            if status["state"] != "open":
                return {"success": False, "error": f"PR is not open (state: {status['state']})"}

            if not status["mergeable"]:
                return {"success": False, "error": "PR has merge conflicts and cannot be merged"}
            ...
            return {"success": False, "error": f"Failed to merge PR: {r.status_code} - {r.text}"}
        except Exception as e:
    ...
    def list_open_prs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to list PRs: {r.status_code} - {r.text}"}
        except Exception as e:
    ...
    def delete_branch(self, branch_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to delete branch: {r.status_code} - {r.text}"}
        except Exception as e:
    ...
    def _create_branch(self, branch_name: str, base_branch: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not base_sha:
                return {"success": False, "error": f"Failed to get base branch SHA for '{base_branch}'"}
            ...
            if r.status_code == 422 and "Reference already exists" in r.text:
                return {"success": True, "branch_name": branch_name, "sha": base_sha}
            return {"success": False, "error": f"Failed to create branch: {r.status_code} - {r.text}"}
        except Exception as e:
            logger.error(f"Error creating branch: {e}")
            return {"success": False, "error": f"Branch creation failed: {e}"}

    def _commit_to_branch(self, branch_name: str, terraform_code: str, commit_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if r.status_code != 200:
                return {"success": False, "error": f"Failed to get current commit: {r.status_code} - {r.text}"}
            ...
            if r.status_code != 201:
                return {"success": False, "error": f"Failed to create tree: {r.status_code} - {r.text}"}
            ...
            r = requests.post(
                ...
                headers=self.headers,
                json={"message": commit_message, "tree": new_tree_sha, "parents": [ref_sha]},
                timeout=30,
            )
            if r.status_code != 201:
                return {"success": False, "error": f"Failed to create commit: {r.status_code} - {r.text}"}
            ...
            return {"success": False, "error": f"Failed to update branch reference: {r.status_code} - {r.text}"}
        except Exception as e:
            logger.error(f"Error committing to branch: {e}")
            return {"success": False, "error": f"Commit failed: {e}"}

    def _commit_multiple_files(self, branch_name: str, files: Dict[str, str], commit_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if r.status_code == 200:
                return {"success": True, "commit_sha": new_commit_sha, "files_committed": list(files.keys())}
            return {"success": False, "error": f"Failed to update branch reference: {r.status_code} - {r.text}"}
        except Exception as e:
    ...
    def _create_blob(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to create blob: {r.status_code} - {r.text}"}
        except Exception as e:
    ...
    def _create_pull_request(
        self, branch_name: str, base_branch: str, title: str, checks_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if r.status_code == 201:
                pr = r.json()
                return {"success": True, "pr_number": pr["number"], "pr_url": pr["html_url"], "pr_id": pr["id"]}
            return {"success": False, "error": f"Failed to create PR: {r.status_code} - {r.text}"}
        except Exception as e:
    ...
    def _generate_pr_description(self, checks_result: Dict[str, Any]) -> str:
        ...
        parts.append("[OK] **Policy Validation**: Passed" if policy.get("passed") else "[ERROR] **Policy Validation**: Failed")
        if not policy.get("passed") and policy.get("violations"):
            parts.extend(["", "**Policy Violations:**"])
            for v in policy["violations"][:5]:
                parts.append(f"- [{v.get('severity','UNKNOWN')}] {v.get('message','Unknown violation')}")
            if len(policy["violations"]) > 5:
                parts.append(f"- ... and {len(policy['violations']) - 5} more violations")
        ...
        if cost.get("passed"):
            parts.append(f"[OK] **Cost Estimation**: ${cost.get('estimated_cost',0):.2f}/month (within budget)")
        else:
            parts.append(
        ...
        parts.append("[OK] **Terraform Validation**: Passed" if val.get("passed") else "[ERROR] **Terraform Validation**: Failed")
        if not val.get("passed") and val.get("errors"):
            parts.extend(["", "**Validation Errors:**", f"```\n{val.get('errors')}\n```"])
...
async def trufflehog_scan_ref(ref: str = None):
    ...
    rc, out, err = await _gh_run_cmd_async(
        _GH.TRUFFLEHOG_BIN, "github", "--repo", _GH.GITHUB_REPO, "--branch", ref, "--json"
    )
    if rc != 0:
...
"""
    ...
    return await commit_files(
        base, {".github/workflows/terraform-apply.yml": workflow}, "chore: enable auto apply on merge"
    )
